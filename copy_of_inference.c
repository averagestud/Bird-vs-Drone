...
#include <math.h>
#include <time.h>
#include "model_weights_uint8.h"  // Contains conv1_weight, bn1_bias, etc.
#include <stdint.h>
...
int main(int argc, char** argv) {
    ...
    size_t num_elements = IMG_CHANNELS * IMG_HEIGHT * IMG_WIDTH;
    ...
    clock_t end_block1 = clock();
    double time_block1 = ((double)(end_block1 - start_block1)) / CLOCKS_PER_SEC;
    printf("Block 1 Inference time: %.6f seconds\n", time_block1);
    
    // ----------------------------
    // Block 2: Conv2 -> BN2 -> ReLU -> MaxPool
    // ----------------------------
    clock_t start_block2 = clock();
    ...
    clock_t end_block2 = clock();
    double time_block2 = ((double)(end_block2 - start_block2)) / CLOCKS_PER_SEC;
    printf("Block 2 Inference time: %.6f seconds\n", time_block2);
    
    // ----------------------------
    // Block 3: Conv3 -> BN3 -> ReLU -> MaxPool
    // ----------------------------
    clock_t start_block3 = clock();
    ...
    clock_t end_block3 = clock();
    double time_block3 = ((double)(end_block3 - start_block3)) / CLOCKS_PER_SEC;
    printf("Block 3 Inference time: %.6f seconds\n", time_block3);
    // ----------------------------
    // Global Average Pooling: reduce each (POOL3_HEIGHT x POOL3_WIDTH) feature map to one value per channel.
    // ----------------------------
    clock_t start_gap = clock();
    ...
    clock_t end_gap = clock();
    double time_gap = ((double)(end_gap - start_gap)) / CLOCKS_PER_SEC;
    printf("Global Average Pooling and Fully Connected Layer Inference time: %.6f seconds\n", time_gap);
    // ----------------------------
